...
public class CustomNodesWithNonSpecifiedProperties {
	...
	public static void main(String[] args)
		...
		try {
			CndImporter.registerNodeTypes(new FileReader("src/main/resources/articleAddNonSpecifiedProperties.cnd"),
					session);
			Node root = session.getRootNode();
			Node articlesNonSpecified = root.addNode("articlesDV");
			Node newNode = articlesNonSpecified.addNode("newNode", "ca:articlensv");
			newNode.setProperty("otherProperty",
					"We just set a new property that was not defined in the node type definition");
			dumpToConsole(newNode);
			session.save();
